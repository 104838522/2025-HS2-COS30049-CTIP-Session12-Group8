...
DATA_PATH = "processed_dataset_final4.csv"  # change if needed
#⚠️⚠️⚠️⚠️⚠️⚠️⚠️⚠️⚠️⚠️⚠️⚠️⚠️
...
#✔️
unique_type = df["vulnerability_type"].unique()
print("\nnumber of CWEs:")
print(unique_type.size)
...
def lang_distribution():
    ...
    ax.set_ylabel("Count")
    annotate_counts_and_pct(ax, totals=counts.sum())
    plt.xticks(rotation=45, ha="right")
    plt.tight_layout()
    plt.show()
...
#✔️
def c_vs_cplus():
    #only works if lang_C and lang_C++ exist
    if "lang_C" not in df.columns or "lang_C++" not in df.columns:
        print("Required columns lang_C or lang_C++ are missing.")
        return
    ...
    ax.set_title("C vs C++ Distribution")
    ax.set_xlabel("Language")
    ax.set_ylabel("Count")
    annotate_counts_and_pct(ax, totals=counts.sum())
...
def top_cwes(top_n=15):
    #✔️
    vc = df["vulnerability_type"].value_counts(dropna=False)
    ...
    ax.set_ylabel("")
    ax.set_title(f"Top {top_n} Vulnerable types (+ Other)")
    plt.tight_layout()
    plt.show()
...
def safe_vul_per_lang():
    ...
    # ✔️
    tmp = df.melt(id_vars=["label_encoded"], value_vars=lang_cols,
                  var_name="Language", value_name="Flag")
    tmp = tmp[tmp["Flag"] == 1]  # keep only rows where language flag is 1
...
def av_tfidf_weight():
    keyword_cols = [
        ...
        if col
        #✔️
        not in ["id", "vulnerability_type", "label_encoded"] + [c for c in df.columns if c.startswith("lang_")]
...
def label_only_corr_heatmap(df, top_k=15, label_col="label_encoded"):
    excl = {"id", label_col, "vulnerability_type"} | {c for c in df.columns if c.startswith("lang_")}
...
def function_length(df, label_col="label_encoded", top_n=2):
    ...
    meta = {"id", label_col, "vulnerability_type"} | {c for c in df.columns if c.startswith("lang_")}
    num_cols = [
...
# sum of tf-idf weights for safe vs vulnerable records

def tfidf_sum_by_class(df, label_col="label_encoded", top_n=15):
    
    meta = {"id", label_col, "vulnerability_type"} | {c for c in df.columns if c.startswith("lang_")}
    num_cols = [c for c in df.select_dtypes(include=[np.number]).columns if c not in meta]

    if not num_cols:
        print("No token columns found.")
        return

    # Safe(0), Vulnerable(1)  TF-IDF sums
    sums = df.groupby(label_col)[num_cols].sum().T
    sums.columns = ["Safe", "Vulnerable"]

    # extract top N tokens by total sum
    sums["Total"] = sums["Safe"] + sums["Vulnerable"]
    top_tokens = sums.sort_values("Total", ascending=False).head(top_n)

    #  visualization
    ax = top_tokens[["Safe", "Vulnerable"]].plot(
        kind="bar", figsize=(12, 6), alpha=0.8
    )
    ax.set_title(f"Top {top_n} Tokens by TF-IDF Sum (Safe vs Vulnerable)")
    ax.set_xlabel("Token")
...
def main():
    top_tokens = tfidf_sum_by_class(df, label_col="label_encoded", top_n=15)
    print(top_tokens)
